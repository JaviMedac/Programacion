...
public class Cuenta {
    ...
    public int retira(int q, String cc) { //retira si la contraseña coincide y hay fondos  
        if (contra.equals(cc)) {
            if (saldo >= q) //saldo > cantidad a retirar
            {
                return q; //Muestro la cantidad a retirar
            } else {
                return -1;  //Si saldo es menor que cantidad a retirar -1
            }
        } else {
            return -2;
        }
    ...
    public int dimeSaldo() { //verifica saldo  
        return saldo;
    }

    public String dimeContra() { //verifica contraseña  
        return contra;
    }
